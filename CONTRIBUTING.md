...
Note that this package is in an initially stable state of development,
with a great deal of active subsequent development envisioned.
...
not a `.Rd` file. You can find the `.R` file that generates the `.Rd` in
the comment in the first line of code.
...
- Fork the package and clone it onto your computer. If you haven’t done
  this before, we recommend using
  `usethis::create_from_github("ropensci/rangr", fork = TRUE)`.
...
- New code should follow the rOpenSci [style
  suggestions](https://devguide.ropensci.org/building.html#code-style).
...
Please note that the rangr project is released with a [Contributor Code
